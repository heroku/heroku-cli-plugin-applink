...
export default abstract class extends Command {
  ...
  get applinkClient(): APIClient {
    if (this._applink) return this._applink;
  ...
  protected getConfigVars(
    addonAttachment: Heroku.AddOnAttachment,
    configVars: Heroku.ConfigVars
  ): { apiUrl: string; applinkToken: string } {
  ...
  protected async configureAppLinkClient(
    app: string,
    addon?: string
  ): Promise<void> {
    if (this._applink) return;

    const appInfoRequest = this.heroku.get<Heroku.App>(`/apps/${app}`);
    const addonsRequest = this.heroku.get<Required<Heroku.AddOn>[]>(
      `/apps/${app}/addons`
    );
    const addonAttachmentsRequest = this.heroku.get<
      Required<Heroku.AddOnAttachment>[]
    >(`/apps/${app}/addon-attachments`);
    const configVarsRequest = this.heroku.get<Heroku.ConfigVars>(
      `/apps/${app}/config-vars`
    );
    const [
      { body: appInfo },
      { body: addons },
      { body: addonAttachments },
      { body: configVars },
    ] = await Promise.all([
      appInfoRequest,
      addonsRequest,
      addonAttachmentsRequest,
      configVarsRequest,
    ]);
    const applinkAddons = addons.filter(
      (addon) => addon.addon_service.name === this.addonServiceSlug
    );
    ...
    if (applinkAddons.length === 0) {
      ...
      applinkAddon = applinkAddons.find(
        (a) => a.name === addon || a.id === addon
      );
      if (!applinkAddon) {
        ux.error(
          heredoc`
    ...
    const addonAttachment = addonAttachments.find(
      (attachment) => attachment.addon.id === applinkAddon.id
    );
    const { apiUrl, applinkToken } = this.getConfigVars(
      addonAttachment!,
      configVars
    );
    ...
    const { body: sso } = await this.heroku.get<HerokuSDK.SSO>(
      `/apps/${app}/addons/${applinkAddon.id}/sso`,
      {
        headers: {
