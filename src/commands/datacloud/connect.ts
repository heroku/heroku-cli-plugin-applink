...
export default class Connect extends Command {
  ...
  static args = {
    connection_name: Args.string({
      required: true,
      description:
        "name for the Data Cloud connection. Must begin with a letter, end with a letter or a number, and between 3-30 characters. Only alphanumeric characters and non-consecutive underscores ('_') are allowed.",
    }),
  };
  ...
  public async run(): Promise<void> {
    ...
    ux.action.start(
      `Connecting Data Cloud org to ${color.app(app)} as ${color.yellow(connectionName)}`
    );
