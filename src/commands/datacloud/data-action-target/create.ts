...
export default class Create extends Command {
  ...
  static args = {
    label: Args.string({
      required: true,
      description:
        "label for the data action target. Must begin with a letter, end with a letter or a number, and between 3-30 characters. Only alphanumeric characters and non-consecutive underscores ('_') are allowed.",
    }),
  };
  ...
  public async run(): Promise<void> {
    ...
    ux.action.start(
      `Creating ${color.app(app)} as '${color.yellow(label)}' data action target ${type} to ${color.yellow(connectionName)}`
    );
