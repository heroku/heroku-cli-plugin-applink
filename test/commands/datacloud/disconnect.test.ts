...
describe('datacloud:disconnect', function () {
  ...
  context('when config var is set to HEROKU_APPLINK_API_URL', function () {
    ...
    it('shows the expected output after failing', async function () {
      applinkApi
        .get(
          '/addons/01234567-89ab-cdef-0123-456789abcdef/connections/datacloud/myorg/data_action_targets'
        )
        .reply(200, [])
        .delete(
          '/addons/01234567-89ab-cdef-0123-456789abcdef/connections/myorg'
        )
        .reply(202, connection5_disconnection_failed);

      try {
        await runCommand(Cmd, ['myorg', '--app=my-app', '--confirm=myorg']);
      } catch (error: unknown) {
    ...
    it('errors when the wrong org name is passed to the confirm flag', async function () {
      applinkApi
        ...
        .reply(200, []);
      try {
        await runCommand(Cmd, ['myorg', '--app=my-app', '--confirm=myorg2']);
      } catch (error: unknown) {
        const { message, oclif } = error as CLIError;
        expect(stripAnsi(message)).to.equal(
          "Confirmation myorg2 doesn't match myorg. Re-run this command to try again."
        );
        expect(oclif.exit).to.equal(1);
      }
    });
    ...
    it('prompts with DAT table when data action targets exist (no indent)', async function () {
      ...
      stdOutputMockStart();
      await runCommand(Cmd, ['myorg', '--app=my-app']);
      stdOutputMockStop();
    ...
    it('prompts without DAT table when no data action targets exist', async function () {
      ...
      expect(stderr.output).to.contain(
        'This command disconnects the org myorg'
      );
